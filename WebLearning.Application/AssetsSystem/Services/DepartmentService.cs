...
namespace WebLearning.Application.Assets.Services
{
    public interface IDepartmentService
    {
        ...
        Task<IEnumerable<AssetsDepartmentDto>> GetAssetDepartmentByParentCode(GetListPagingRequest getListPagingRequest);
    ...
    public class DepartmentService : IDepartmentService
    {
        ...
        public async Task<IEnumerable<AssetsDepartmentDto>> GetAssetDepartmentByParentCode(GetListPagingRequest getListPagingRequest)
        {
            ...
            var asset = await _context.AssetsDepartments.AsNoTracking().ToListAsync();

            if (getListPagingRequest.AssetsDepartmentLocationId != null && getListPagingRequest.AssetsDepartmentLocationId.Length > 0)
            {
                asset = asset.Where(x => getListPagingRequest.AssetsDepartmentLocationId.Contains(x.ParentCode) && x.Level == "2").ToList();
            }
            return _mapper.Map<List<AssetsDepartmentDto>>(asset);
        }
        ...
        {
            var asset = await _context.AssetsDepartments.FirstOrDefaultAsync(x => x.Code.Contains(code));
        ...
        public async Task<PagedViewModel<AssetsDepartmentDto>> GetPaging([FromQuery] GetListPagingRequest getListPagingRequest)
        {
            ...
            {
                var nameLo = await GetCode(item.ParentCode);
                if (nameLo == null) { continue; };
                item.ParentCode = nameLo.Name;
            }
        ...
        public async Task InsertAssetsDepartment(CreateAssetsDepartmentDto createAssetsDepartmentDto)
        {
            ...
            if (_context.AssetsDepartments.Any(x => x.Name.Equals(createAssetsDepartmentDto.Name)) == false)
            {
                ...
                if (createAssetsDepartmentDto.Location == true)
                {
                    createAssetsDepartmentDto.Level = "1";
                    createAssetsDepartmentDto.ParentCode = createAssetsDepartmentDto.Code;
                }
                else
                {
                    createAssetsDepartmentDto.Level = "2";

                }

                createAssetsDepartmentDto.Id = Guid.NewGuid();
                AssetDepartment asset = _mapper.Map<AssetDepartment>(createAssetsDepartmentDto);
